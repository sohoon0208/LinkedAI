...
evidence, and fresh snapshot; independently decides whether the work is done.
...
4. LUNA MAX EXECUTION: execute only an ASTRA `IMPLEMENT` plan for a `change`
   intent; keep investigation/review/explanation read-only. Run the planned
   checks and gather a fresh snapshot.
5. SOL HIGH VERIFY: independently inspect the packet, plan, result, evidence,
   and snapshot. Return `DONE`, `RETRY`, `REPLAN`, `EVIDENCE`, or `BLOCKED`.
6. If SOL does not return `DONE`, follow only its named bounded transition.
   ...
   two LUNA attempts, two SOL verifications, or two ASTRA plans are exhausted.
